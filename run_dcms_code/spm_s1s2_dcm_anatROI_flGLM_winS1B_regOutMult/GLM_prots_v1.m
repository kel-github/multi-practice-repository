function GLM_prots_v1(sub, n_s1_runs, n_s2_runs, s1_runs, s2_runs, fpath, sfol)
...
count_adds = 0;
    for x = 1:n_s1_runs
        ...
        load(sprintf(['Training_fMRI_%d%d_run0%d.mat'],sub,1,s1_runs(x)));
            if x == 1
                ...
                all_ons{2} = onsets{2};
                all_ons{3} = onsets{3};
                all_durs{1} = durations{1};
                all_durs{2} = durations{2};
                all_durs{3} = durations{3};
            else
                ...
                all_ons{1} = [all_ons{1} onsets{1}];
                all_ons{2} = [all_ons{2} onsets{2}];
                all_ons{3} = [all_ons{3} onsets{3}];
                all_durs{1} = [all_durs{1} durations{1}];
                all_durs{2} = [all_durs{2} durations{2}];
                all_durs{3} = [all_durs{3} durations{3}];
            end
    end
    ...
    for x = 1:n_s2_runs
        ...
        load(sprintf(['Training_fMRI_%d%d_run0%d.mat'],sub,2,s2_runs(x)));
            ...
            % as above, allocate to condition regressor
            all_ons{1} = [all_ons{1} onsets{1}];
            all_ons{2} = [all_ons{2} onsets{2}];
            all_ons{3} = [all_ons{3} onsets{3}];
            all_durs{1} = [all_durs{1} durations{1}];
            ...
            % now take all the onsets from the s2 session for the practice regressor
            if x == 1
                all_ons{4} = [onsets{1} onsets{2} onsets{3}];
                all_durs{4} = [durations{1} durations{2} durations{3}];

            else
                all_ons{4} = [all_ons{4} onsets{1} onsets{2} onsets{3}];
                all_durs{4} = [all_durs{4} durations{1} durations{2} durations{3}];
            end
    end
...
names = {'vm','am','multi','practice'};
onsets = all_ons;
durations = all_durs;

save(sprintf([fpath '/' sfol '/FSTL_GLM/%d_DCMSPMSESS_onsets.mat'],sub,sub),'names','durations','onsets');
