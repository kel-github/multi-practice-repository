...
%%%%%%% "PRACTICE" and in this folder, then removes the multi, so that only
%%%%%%% single onsets and practice regressors remain
...
count_adds = 0;
    for x = 1:n_s1_runs
        ...
        load(sprintf(['Training_fMRI_%d%d_run0%d.mat'],sub,1,s1_runs(x)));
            if x == 1
                ...
                % now take the onsets of all events (for regressor: SINGLE)
                all_ons{1} = [onsets{1} onsets{2}];
                all_durs{1} = [durations{1} durations{2}];
            else
                ...
                all_ons{1} = [all_ons{1} onsets{1} onsets{2}];
                all_durs{1} = [all_durs{1} durations{1} durations{2}];
            end
    end
    ...
    for x = 1:n_s2_runs
        ...
        load(sprintf(['Training_fMRI_%d%d_run0%d.mat'],sub,2,s2_runs(x)));
            ...
            if x == 1
                all_ons{2} = [onsets{1} onsets{2}];
                all_durs{2} = [durations{1} durations{2}];

            else
                all_ons{2} = [all_ons{2} onsets{1} onsets{2}];
                all_durs{2} = [all_durs{2} durations{1} durations{2}];
            end
     end
