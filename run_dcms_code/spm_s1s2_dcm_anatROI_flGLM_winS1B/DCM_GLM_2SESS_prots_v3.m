function DCM_GLM_2SESS_prots_v2(sub,n_s1_runs, n_s2_runs, s1_runs, s2_runs, fpath,sfol)
...
count_adds = 0;
    for x = 1:n_s1_runs
        ...
        load(sprintf(['Training_fMRI_%d%d_run0%d.mat'],sub,1,s1_runs(x)));
            if x == 1
                ...
                % take the onsets of the multitask events for regressor:
                % MULTI
                all_ons{2} = onsets{3};
                ...
                % and take the onsets of the multiask for the MULTI
                % regressor
    ...
    for x = 1:n_s2_runs
        ...
        load(sprintf(['Training_fMRI_%d%d_run0%d.mat'],sub,2,s2_runs(x)));
            ...
            if x == 1
                all_ons{3} = [onsets{1} onsets{2} onsets{3}];
                all_durs{3} = [durations{1} durations{2} durations{3}];

            else
                all_ons{3} = [all_ons{3} onsets{1} onsets{2} onsets{3}];
                all_durs{3} = [all_durs{3} durations{1} durations{2} durations{3}];
            end
     end

names = {'all','multi','practice'};
onsets = all_ons;
durations = all_durs;
