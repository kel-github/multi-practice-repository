...
%%%%%%%%%%%% this version regresses out single task trials to look at the
%%%%%%%%%%%% influence of practice on multitask trials
clear all
%start = tic
tmpdir = getenv('TMPDIR');
%tmpdir = pwd;
sfol = 'sub_%d_out_s1s2_anatROI_initGLM_regOutSing';
...
load('b_mats_v5'); % all perms of 5 combos
b_mats(:,:,32:end) = []; % remove erroneously duplicated models
...
if define_cl_glm
    ...
    for crun = 1:n_s2_runs
      ...
      sub_fol =  sprintf('sub_%d%d_out',subs(arr_num),2);
...
if define_dcms
    ...
    DCM.c = [0 0; ...
         ...
         for count_prac_mats = 1:total_prac_mats
             clear matlabbatch
             DCM.b = zeros(3,3,2); % base 
             DCM.b(:,:,2) = b_mats(:,:,count_prac_mats); % add mod practice
